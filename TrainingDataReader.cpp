...
namespace BPN
{
    TrainingDataReader::TrainingDataReader( std::string const& filename, std::string const& inputfilename1, int32_t numInputs, int32_t numOutputs )
        : m_filename( filename )
		, m_inputfilename1(inputfilename1)
        , m_numInputs( numInputs )
        , m_numOutputs( numOutputs )
    {
        assert( !filename.empty()&&!inputfilename1.empty() && m_numInputs > 0 && m_numOutputs > 0 );
    }

    bool TrainingDataReader::ReadData()
    {
        assert( !m_filename.empty()&& !m_inputfilename1);

        std::fstream inputFile;
		std::fstream inputFile1;
        inputFile.open( m_filename, std::ios::in );
        inputFile1.open(m_inputfilename1, std::ios::in );
        ...
        if ( inputFile.is_open() )
        {
            ...
            int32_t const totalValuesToRead = m_numInputs ;
